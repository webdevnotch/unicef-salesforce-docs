/**
 * Helper function to create GitHub social links with custom labels
 */

// GitHub icon path (reusable)
...
/**
 * Create a GitHub social link with custom title and aria label
 */
export function createGitHubLink(url: string, title: string, ariaLabel: string) {
  return {
    icon: {
      svg: `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>${title}</title><path d="${githubIconPath}"/></svg>`
    },
    link: url,
    ariaLabel: ariaLabel
  }
